...
static struct platform_device deluxe_u_android_pmem_device = {
	.name = "android_pmem",
	.id = 0,
...
static struct platform_device deluxe_u_android_pmem_adsp_device = {
	.name = "android_pmem",
	.id = 2,
	.dev = { .platform_data = &android_pmem_adsp_pdata },
};

static struct android_pmem_platform_data deluxe_u_android_pmem_audio_pdata = {
	.name = "pmem_audio",
	.allocator_type = PMEM_ALLOCATORTYPE_BITMAP,
	.cached = 0,
	.memory_type = MEMTYPE_EBI1,
...
static struct msm_rtb_platform_data deluxe_u_rtb_pdata = {
...
static int __init msm_rtb_set_buffer_size(char *p)
{
       ...
       s = memparse(p, NULL);
       deluxe_u_rtb_pdata.size = ALIGN(s, SZ_4K);
       return 0;
}
early_param("msm_rtb_size", msm_rtb_set_buffer_size);


static struct platform_device deluxe_u_rtb_device = {
       .name           = "msm_rtb",
       .id             = -1,
       .dev            = {
               .platform_data = &deluxe_u_rtb_pdata,
       },
};
#endif
...
static int deluxe_u_paddr_to_memtype(unsigned int paddr)
{
	return MEMTYPE_EBI1;
}
...
#ifdef CONFIG_MSM_MULTIMEDIA_USE_ION
static struct ion_cp_heap_pdata cp_mm_deluxe_u_ion_pdata = {
...
static struct ion_cp_heap_pdata cp_mfc_deluxe_u_ion_pdata = {
...
static struct ion_co_heap_pdata co_deluxe_u_ion_pdata = {
	.adjacent_mem_id = INVALID_HEAP_ID,
	.align = PAGE_SIZE,
	.mem_is_fmem = 0,
};

static struct ion_co_heap_pdata fw_co_deluxe_u_ion_pdata = {
...
static struct ion_platform_data ion_pdata = {
	...
	.heaps = {
...
#ifdef CONFIG_MSM_MULTIMEDIA_USE_ION
		{
			...
			.extra_data = (void *) &cp_mm_deluxe_u_ion_pdata,
		},
		{
			...
			.memory_type = ION_EBI_TYPE,
			.extra_data = (void *) &fw_co_deluxe_u_ion_pdata,
		},
		{
			...
			.memory_type = ION_EBI_TYPE,
			.extra_data = (void *) &cp_mfc_deluxe_u_ion_pdata,
		},
		{
			.id	= ION_SF_HEAP_ID,
			.type	= ION_HEAP_TYPE_CARVEOUT,
			...
			.memory_type = ION_EBI_TYPE,
			.extra_data = (void *) &co_deluxe_u_ion_pdata,
		},
		{
...
static struct platform_device deluxe_u_ion_dev = {
	.name = "ion-msm",
	.id = 1,
...
static struct platform_device deluxe_u_rfkill = {
	.name = "deluxe_u_rfkill",
	.id = -1,
};
#endif
...
{
	deluxe_u_mdp_writeback(apq8064_reserve_table);
}

static void __init deluxe_u_calculate_reserve_sizes(void)
{
	size_pmem_devices();
	reserve_pmem_memory();
	reserve_ion_memory();
...
static struct reserve_info deluxe_u_reserve_info __initdata = {
	.memtype_reserve_table = apq8064_reserve_table,
	.calculate_reserve_sizes = deluxe_u_calculate_reserve_sizes,
	.reserve_fixed_area = apq8064_reserve_fixed_area,
	.paddr_to_memtype = deluxe_u_paddr_to_memtype,
};

static int deluxe_u_memory_bank_size(void)
{
	return 1<<29;
}
...
static void __init locate_unstable_memory(void)
{
	...
	bank_size = deluxe_u_memory_bank_size();
	...
	deluxe_u_reserve_info.low_unstable_address = mb->start -
					MIN_MEMORY_BLOCK_SIZE + mb->size;
	deluxe_u_reserve_info.max_unstable_size = MIN_MEMORY_BLOCK_SIZE;

	deluxe_u_reserve_info.bank_size = bank_size;
	pr_info("low unstable address %lx max size %lx bank size %lx\n",
		deluxe_u_reserve_info.low_unstable_address,
		deluxe_u_reserve_info.max_unstable_size,
		deluxe_u_reserve_info.bank_size);
	return;
#endif
no_dmm:
	deluxe_u_reserve_info.low_unstable_address = high;
	deluxe_u_reserve_info.max_unstable_size = 0;
}

int __init parse_tag_memsize(const struct tag *tags);
static unsigned int mem_size_mb;

static void __init deluxe_u_reserve(void)
{
	if (mem_size_mb == 64)
		return;
...
static void __init deluxe_u_early_reserve(void)
{
	reserve_info = &deluxe_u_reserve_info;
...
static void deluxe_u_usb_dpdn_switch(int path)
{
	switch (path) {
	case PATH_USB:
...
static T_MHL_PLATFORM_DATA mhl_sii9234_device_data = {
	...
	.ci2ca = 0,
#ifdef CONFIG_FB_MSM_HDMI_MHL
	.mhl_usb_switch		= deluxe_u_usb_dpdn_switch,
...
static int64_t deluxe_u_get_usbid_adc(void)
{
...
static void deluxe_u_config_usb_id_gpios(bool output)
{
	int rc;
...
static struct cable_detect_platform_data cable_detect_pdata = {
       ...
       .usb_id_pin_gpio        = USB1_HS_ID_GPIO,
       .get_adc_cb             = deluxe_u_get_usbid_adc,
       .config_usb_id_gpios    = deluxe_u_config_usb_id_gpios,
#ifdef CONFIG_FB_MSM_HDMI_MHL
       .mhl_1v2_power = mhl_sii9234_1v2_power,
       .usb_dpdn_switch        = deluxe_u_usb_dpdn_switch,
#endif
};
...
void deluxe_u_cable_detect_register(void)
{
	int rc;
...
void deluxe_u_pm8xxx_adc_device_register(void)
{
	pr_info("%s: Register PM8XXX ADC device. rev: %d\n",
		__func__, system_rev);
	deluxe_u_cable_detect_register();
}

void deluxe_u_add_usb_devices(void)
{
	printk(KERN_INFO "%s rev: %d\n", __func__, system_rev);

	if (machine_is_deluxe_u())
		android_usb_pdata.products[0].product_id = 0x0de8;
	else if (0)
		android_usb_pdata.products[0].product_id = 0x0de7;
	else
		printk(KERN_INFO "[USB]incorrect machine info, no corresponding product id\n");
...
static struct wcd9xxx_pdata deluxe_u_tabla_platform_data = {
...
static struct slim_device deluxe_u_slim_tabla = {
	.name = "tabla-slim",
	.e_addr = {0, 1, 0x10, 0, 0x17, 2},
	.dev = {
		.platform_data = &deluxe_u_tabla_platform_data,
	},
};

static struct wcd9xxx_pdata deluxe_u_tabla20_platform_data = {
	.slimbus_slave_device = {
		.name = "tabla-slave",
...
static struct slim_device deluxe_u_slim_tabla20 = {
	.name = "tabla2x-slim",
	.e_addr = {0, 1, 0x60, 0, 0x17, 2},
	.dev = {
		.platform_data = &deluxe_u_tabla20_platform_data,
	},
};
...
static void __init deluxe_u_map_io(void)
{
...
static void __init deluxe_u_init_irq(void)
{
...
static struct platform_device msm8064_device_saw_regulator_core0 = {
	...
	.id	= 0,
	.dev	= {
		.platform_data = &deluxe_u_saw_regulator_pdata_8921_s5,
	},
};
...
static struct platform_device msm8064_device_saw_regulator_core1 = {
	...
	.id	= 1,
	.dev	= {
		.platform_data = &deluxe_u_saw_regulator_pdata_8921_s6,
	},
};
...
static struct platform_device msm8064_device_saw_regulator_core2 = {
	...
	.id	= 2,
	.dev	= {
		.platform_data = &deluxe_u_saw_regulator_pdata_8821_s0,
	},
};
...
static struct platform_device msm8064_device_saw_regulator_core3 = {
	...
	.id	= 3,
	.dev	= {
		.platform_data = &deluxe_u_saw_regulator_pdata_8821_s1,
...
static void __init deluxe_u_init_buses(void)
{
...
static struct platform_device deluxe_u_device_ext_5v_vreg __devinitdata = {
	.name	= GPIO_REGULATOR_DEV_NAME,
	.id	= PM8921_MPP_PM_TO_SYS(7),
	.dev	= {
		.platform_data
			= &deluxe_u_gpio_regulator_pdata[GPIO_VREG_ID_EXT_5V],
	},
};

static struct platform_device deluxe_u_device_ext_mpp8_vreg __devinitdata = {
	.name	= GPIO_REGULATOR_DEV_NAME,
	.id	= PM8921_MPP_PM_TO_SYS(8),
	.dev	= {
		.platform_data
			= &deluxe_u_gpio_regulator_pdata[GPIO_VREG_ID_EXT_MPP8],
	},
};

static struct platform_device deluxe_u_device_ext_ts_sw_vreg __devinitdata = {
	.name	= GPIO_REGULATOR_DEV_NAME,
	.id	= PM8921_GPIO_PM_TO_SYS(23),
	.dev	= {
		.platform_data
			= &deluxe_u_gpio_regulator_pdata[GPIO_VREG_ID_EXT_TS_SW],
	},
};

static struct platform_device deluxe_u_device_rpm_regulator __devinitdata = {
	.name	= "rpm-regulator",
	.id	= -1,
	.dev	= {
		.platform_data = &deluxe_u_rpm_regulator_pdata,
	},
};
...
static mhl_driving_params deluxe_u_driving_params[] = {
	{.format = HDMI_VFRMT_640x480p60_4_3,	.reg_a3=0xEA, .reg_a6=0x0C},
	{.format = HDMI_VFRMT_720x480p60_16_9,	.reg_a3=0xEA, .reg_a6=0x0C},
	{.format = HDMI_VFRMT_1280x720p60_16_9,	.reg_a3=0xFE, .reg_a6=0x0C},
	{.format = HDMI_VFRMT_720x576p50_16_9,	.reg_a3=0xEA, .reg_a6=0x0C},
...
static struct msm_hdmi_platform_data hdmi_msm_data = {
	...
	.driving_params = deluxe_u_driving_params,
	.dirving_params_count = ARRAY_SIZE(deluxe_u_driving_params),
};
...
static struct platform_device *common_devices[] __initdata = {
	...
	&msm_device_uart_dm4,
#endif
	&deluxe_u_device_ext_5v_vreg,
	&deluxe_u_device_ext_mpp8_vreg,
	&deluxe_u_device_ext_ts_sw_vreg,
...
#ifdef CONFIG_ANDROID_PMEM
#ifndef CONFIG_MSM_MULTIMEDIA_USE_ION
	&deluxe_u_android_pmem_device,
	&deluxe_u_android_pmem_adsp_device,
	&deluxe_u_android_pmem_audio_device,
#endif 
#endif 
#ifdef CONFIG_ION_MSM
	&deluxe_u_ion_dev,
...
static struct msm_spi_platform_data deluxe_u_qup_spi_gsbi5_pdata = {
...
static struct slim_boardinfo deluxe_u_slim_devices[] = {
	{
		.bus_num = 1,
		.slim_slave = &deluxe_u_slim_tabla,
	},
	{
		.bus_num = 1,
		.slim_slave = &deluxe_u_slim_tabla20,
	},
	
};

static struct msm_i2c_platform_data deluxe_u_i2c_qup_gsbi1_pdata = {
	.clk_freq = 400000,
...
static struct msm_i2c_platform_data deluxe_u_i2c_qup_gsbi3_pdata = {
	.clk_freq = 400000,
	.src_clk_rate = 24000000,
...
static struct msm_i2c_platform_data deluxe_u_i2c_qup_gsbi4_pdata = {
	.clk_freq = 400000,
	.src_clk_rate = 24000000,
	.share_uart_flag = 1,
};
...
int deluxe_irda_enable(int ebl)
{
	...
	if (ebl) {
		ioext_i2c_read(0x24, &val, 1);
		val &= ~0x10; 
		ioext_i2c_write(0x24, &val, 1);
		usleep(100);
		ioext_i2c_read(0x24, &val, 1);
		val |= 0x10;
		ioext_i2c_write(0x24, &val, 1);
...
static struct irda_platform_data deluxe_u_irda_gsbi3_pdata = {
...
static void __init deluxe_u_i2c_init(void)
{
	void __iomem *gsbi_mem;

	apq8064_device_qup_i2c_gsbi1.dev.platform_data =
					&deluxe_u_i2c_qup_gsbi1_pdata;
	gsbi_mem = ioremap_nocache(MSM_GSBI1_PHYS, 4);
	writel_relaxed(GSBI_DUAL_MODE_CODE, gsbi_mem);
	/* Ensure protocol code is written before proceeding */
	wmb();
	iounmap(gsbi_mem);
	deluxe_u_i2c_qup_gsbi1_pdata.use_gsbi_shared_mode = 1;

	apq8064_device_qup_i2c_gsbi2.dev.platform_data =
					&apq8064_i2c_qup_gsbi2_pdata;
	apq8064_device_qup_i2c_gsbi3.dev.platform_data =
					&deluxe_u_i2c_qup_gsbi3_pdata;
	apq8064_device_qup_i2c_gsbi4.dev.platform_data =
					&deluxe_u_i2c_qup_gsbi4_pdata;
}
...
static void __init deluxe_u_init_dsps(void)
{
...
static struct i2c_registry deluxe_u_i2c_devices[] __initdata = {
	{
		I2C_SURF | I2C_FFA,
...
static void __init register_i2c_devices(void)
{
...
#ifdef CONFIG_MSM_CAMERA
	struct i2c_registry deluxe_u_camera_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_LIQUID | I2C_RUMI,
		APQ_8064_GSBI4_QUP_I2C_BUS_ID,
		deluxe_u_camera_board_info.board_info,
		deluxe_u_camera_board_info.num_i2c_board_info,
	};
#endif
	...
	for (i = 0; i < ARRAY_SIZE(deluxe_u_i2c_devices); ++i) {
		if (deluxe_u_i2c_devices[i].machs & mach_mask)
			i2c_register_board_info(deluxe_u_i2c_devices[i].bus,
						deluxe_u_i2c_devices[i].info,
						deluxe_u_i2c_devices[i].len);
	}
#ifdef CONFIG_MSM_CAMERA
	if (system_rev == XA) {
		if (deluxe_u_camera_i2c_devices.machs & mach_mask)
			i2c_register_board_info(deluxe_u_camera_i2c_devices.bus,
				deluxe_u_camera_i2c_devices.info,
				deluxe_u_camera_i2c_devices.len);
	}
#endif
	if (gy_type == 2) {
...
static void __init deluxe_u_irda_init(void)
{
	msm_uart_gsbi3_gpio_init();
	apq8064_device_uart_gsbi3.dev.platform_data =
					&deluxe_u_irda_gsbi3_pdata;
}
#endif

static void __init deluxe_u_common_init(void)
{
	int rc = 0;
	...
	platform_device_register(&deluxe_u_device_rpm_regulator);
	if (msm_xo_init())
		pr_err("Failed to initialize XO votes\n");
	
	clk_ignor_list_add("msm_sdcc.3", "core_clk", &deluxe_u_clock_init_data_xb);
	
	msm_clock_init(&deluxe_u_clock_init_data_xb);
	deluxe_u_init_gpiomux();
#ifdef CONFIG_RESET_BY_CABLE_IN
	...
	slim_register_board_info(deluxe_u_slim_devices,
		ARRAY_SIZE(deluxe_u_slim_devices));
	deluxe_u_init_dsps();
...
static void __init deluxe_u_allocate_memory_regions(void)
{
	deluxe_u_allocate_fb_region();
}

static void __init deluxe_u_cdp_init(void)
{
	pr_info("%s: init starts\r\n", __func__);
	msm_tsens_early_init(&apq_tsens_pdata);
	deluxe_u_common_init();
	ethernet_init();
...
static void __init deluxe_u_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
...
MACHINE_START(DELUXE_U, "UNKNOWN")
	.fixup = deluxe_u_fixup,
	.map_io = deluxe_u_map_io,
	.reserve = deluxe_u_reserve,
	.init_irq = deluxe_u_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = deluxe_u_cdp_init,
	.init_early = deluxe_u_allocate_memory_regions,
	.init_very_early = deluxe_u_early_reserve,
	.restart = msm_restart,
MACHINE_END
