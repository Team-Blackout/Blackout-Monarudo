...
static int deluxe_u_detect_panel(const char *name)
{
#if 0
...
static struct msm_fb_platform_data msm_fb_pdata = {
	.detect_client = deluxe_u_detect_panel,
};
...
void __init deluxe_u_allocate_fb_region(void)
{
	void *addr;
	unsigned long size;
...
void __init deluxe_u_mdp_writeback(struct memtype_reserve* reserve_table)
{
...
static struct mipi_dsi_panel_platform_data *mipi_deluxe_u_pdata;

static struct dsi_buf deluxe_u_panel_tx_buf;
static struct dsi_buf deluxe_u_panel_rx_buf;
...
static uint32 mipi_renesas_manufacture_id(struct msm_fb_data_type *mfd)
{
	...
	tp = &deluxe_u_panel_tx_buf;
	rp = &deluxe_u_panel_rx_buf;
...
static int deluxe_u_lcd_on(struct platform_device *pdev)
{
	...
	if (!first_init) {
		if (mipi->mode == DSI_VIDEO_MODE) {
			mipi_dsi_cmds_tx(&deluxe_u_panel_tx_buf, video_on_cmds,
				video_on_cmds_count);
		}
	}
	first_init = 0;
...
static int deluxe_u_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static int __devinit deluxe_u_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
		mipi_deluxe_u_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static void deluxe_u_display_on(struct msm_fb_data_type *mfd)
{
...
static void deluxe_u_display_off(struct msm_fb_data_type *mfd)
{
...
static unsigned char deluxe_u_shrink_pwm(int val)
{
...
static struct platform_driver this_driver = {
	.probe  = deluxe_u_lcd_probe,
	.driver = {
		.name   = "mipi_deluxe_u",
	},
};

static struct msm_fb_panel_data deluxe_u_panel_data = {
	.on	= deluxe_u_lcd_on,
	.off	= deluxe_u_lcd_off,
	.set_backlight = deluxe_u_set_backlight,
	.display_on = deluxe_u_display_on,
	.display_off = deluxe_u_display_off,
};

static struct msm_panel_info pinfo;
static int ch_used[3] = {0};

static int mipi_deluxe_u_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	pdev = platform_device_alloc("mipi_deluxe_u", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	deluxe_u_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &deluxe_u_panel_data,
		sizeof(deluxe_u_panel_data));
	if (ret) {
...
static int __init mipi_video_sharp_init(void)
{
	...
	ret = mipi_deluxe_u_device_register(&pinfo, MIPI_DSI_PRIM,
...
void __init deluxe_u_init_fb(void)
{
...
static int __init deluxe_u_panel_init(void)
{
	int ret;
	...
	mipi_dsi_buf_alloc(&deluxe_u_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&deluxe_u_panel_rx_buf, DSI_BUF_SIZE);
...
device_initcall_sync(deluxe_u_panel_init);
